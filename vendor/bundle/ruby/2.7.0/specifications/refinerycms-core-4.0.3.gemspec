# -*- encoding: utf-8 -*-
# stub: refinerycms-core 4.0.3 ruby lib

Gem::Specification.new do |s|
  s.name = "refinerycms-core".freeze
  s.version = "4.0.3"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Philip Arndt".freeze, "U\u0123is Ozols".freeze, "Rob Yurkowski".freeze]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2018-10-29"
  s.description = "The core of Refinery CMS. This handles the common functionality and is required by most extensions".freeze
  s.email = "refinerycms@p.arndt.io".freeze
  s.homepage = "https://www.refinerycms.com".freeze
  s.licenses = ["MIT".freeze]
  s.required_ruby_version = Gem::Requirement.new(">= 2.2.2".freeze)
  s.rubygems_version = "3.2.17".freeze
  s.summary = "Core extension for Refinery CMS".freeze

  s.installed_by_version = "3.2.17" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4
  end

  if s.respond_to? :add_runtime_dependency then
    s.add_runtime_dependency(%q<refinerycms-i18n>.freeze, ["~> 4.0", ">= 4.0.0"])
    s.add_runtime_dependency(%q<awesome_nested_set>.freeze, ["~> 3.0", ">= 3.0.0"])
    s.add_runtime_dependency(%q<railties>.freeze, [">= 5.1.0", "< 5.2"])
    s.add_runtime_dependency(%q<activerecord>.freeze, [">= 5.1.0", "< 5.2"])
    s.add_runtime_dependency(%q<actionpack>.freeze, [">= 5.1.0", "< 5.2"])
    s.add_runtime_dependency(%q<truncate_html>.freeze, ["~> 0.9"])
    s.add_runtime_dependency(%q<will_paginate>.freeze, ["~> 3.1", ">= 3.1.0"])
    s.add_runtime_dependency(%q<sass-rails>.freeze, [">= 4.0", "< 5.1"])
    s.add_runtime_dependency(%q<font-awesome-sass>.freeze, [">= 4.3.0", "< 5.0"])
    s.add_runtime_dependency(%q<coffee-rails>.freeze, ["~> 4.0", ">= 4.0.0"])
    s.add_runtime_dependency(%q<jquery-rails>.freeze, ["~> 4.3", ">= 4.3.1"])
    s.add_runtime_dependency(%q<jquery-ui-rails>.freeze, ["~> 5.0", ">= 5.0.0"])
    s.add_runtime_dependency(%q<decorators>.freeze, ["~> 2.0", ">= 2.0.0"])
    s.add_runtime_dependency(%q<zilch-authorisation>.freeze, ["~> 0", ">= 0.0.1"])
  else
    s.add_dependency(%q<refinerycms-i18n>.freeze, ["~> 4.0", ">= 4.0.0"])
    s.add_dependency(%q<awesome_nested_set>.freeze, ["~> 3.0", ">= 3.0.0"])
    s.add_dependency(%q<railties>.freeze, [">= 5.1.0", "< 5.2"])
    s.add_dependency(%q<activerecord>.freeze, [">= 5.1.0", "< 5.2"])
    s.add_dependency(%q<actionpack>.freeze, [">= 5.1.0", "< 5.2"])
    s.add_dependency(%q<truncate_html>.freeze, ["~> 0.9"])
    s.add_dependency(%q<will_paginate>.freeze, ["~> 3.1", ">= 3.1.0"])
    s.add_dependency(%q<sass-rails>.freeze, [">= 4.0", "< 5.1"])
    s.add_dependency(%q<font-awesome-sass>.freeze, [">= 4.3.0", "< 5.0"])
    s.add_dependency(%q<coffee-rails>.freeze, ["~> 4.0", ">= 4.0.0"])
    s.add_dependency(%q<jquery-rails>.freeze, ["~> 4.3", ">= 4.3.1"])
    s.add_dependency(%q<jquery-ui-rails>.freeze, ["~> 5.0", ">= 5.0.0"])
    s.add_dependency(%q<decorators>.freeze, ["~> 2.0", ">= 2.0.0"])
    s.add_dependency(%q<zilch-authorisation>.freeze, ["~> 0", ">= 0.0.1"])
  end
end
